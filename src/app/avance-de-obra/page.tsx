...
import { constructionProgress } from '@/data/constructionProgress';
...
const ConstructionProgressPage = () => {
    ...
    const currentUpdate = constructionProgress[currentIndex];
    ...
    const handleNext = () => {
        if (currentIndex < constructionProgress.length - 1) {
        ...
        <div className="h-full w-full bg-black text-white relative overflow-hidden font-sans">
            ...
            <div className="absolute top-6 left-6 z-50 flex items-center gap-4">
                ...
                <div className="flex flex-col">
                    ...
                    <p className="text-[10px] text-gray-400 uppercase tracking-widest font-secondary"> {config.company?.buildingName} </p>
                </div>
            </div>
                ...
                <div className="flex-1 relative bg-neutral-900 overflow-hidden group h-[50vh] md:h-full">
                    ...
                    <div className="absolute inset-y-0 left-0 w-24 hidden md:flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">
                        <button 
                            ...
                            className={`p-4 rounded-full bg-white/10 hover:bg-white/20 backdrop-blur-md border border-white/20 transition-all ${currentIndex === 0 ? 'opacity-0' : 'cursor-pointer hover:scale-110'}`}
                        >
                            <ChevronLeft size={32} />
                        </button>
                    </div>
                    <div className="absolute inset-y-0 right-0 w-24 hidden md:flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">
                        <button 
                            onClick={handleNext}
                            disabled={currentIndex === constructionProgress.length - 1}
                            className={`p-4 rounded-full bg-white/10 hover:bg-white/20 backdrop-blur-md border border-white/20 transition-all ${currentIndex === constructionProgress.length - 1 ? 'opacity-0' : 'cursor-pointer hover:scale-110'}`}
                        >
                ...
                <div className="w-full md:w-[400px] lg:w-[450px] bg-black/90 backdrop-blur-3xl border-l border-white/10 p-8 lg:p-12 flex flex-col justify-between relative z-20 overflow-y-auto">
                    <div ref={contentRef} className="space-y-6 md:mt-24">
                        ...
                        <div className="space-y-4">
                            ...
                            <p className="text-gray-400 font-secondary leading-relaxed text-sm lg:text-base">
                    ...
                    <div className="mt-12 md:mt-auto pt-8 border-t border-white/5">
                        <div className="flex gap-4 overflow-x-auto pb-4 scrollbar-thin">
                            {constructionProgress.map((update, index) => (
                                <button
                                    key={update.id}
                                    onClick={() => setCurrentIndex(index)}
                                    className={`flex-shrink-0 w-32 group cursor-pointer transition-all text-left ${index === currentIndex ? 'opacity-100' : 'opacity-40 hover:opacity-70'}`}
                                >
                                    <div className={`h-1 w-full mb-3 rounded-full transition-all ${index === currentIndex ? 'bg-brand-primary' : 'bg-white/20'}`} />
                                    <p className={`text-[10px] font-bold uppercase tracking-wider mb-1 ${index === currentIndex ? 'text-brand-primary' : 'text-white'}`}>{update.date}</p>
                                    <p className="text-[9px] text-gray-500 line-clamp-1 uppercase tracking-tighter">{update.title}</p>
                                </button>
                            ))}
                        </div>
                        ...
                        <div className="md:hidden flex justify-between items-center pt-4">
                             <button onClick={handlePrev} disabled={currentIndex === 0} className="p-2 bg-white/5 rounded-full disabled:opacity-20"><ChevronLeft size={20} /></button>
                             <span className="text-[10px] font-bold uppercase tracking-[0.2em] text-gray-500">{currentIndex + 1} / {constructionProgress.length}</span>
                             <button onClick={handleNext} disabled={currentIndex === constructionProgress.length - 1} className="p-2 bg-white/5 rounded-full disabled:opacity-20"><ChevronRight size={20} /></button>
                        </div>
                    </div>
                </div>
            </div>
